    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            let packet = CapturedPacket {
                ...
                local_addr: CompactString::from(format!(
                    "{}:{}",
                    local_ip,
                    40000 + (counter % 10000)
                )),
            ...
            if packets_generated % 10 == 0 {
                eprintln!(
                    "[core-engine] Generated demo packet #{} ({} -> {})",
                    packets_generated, packet.local_addr, packet.remote_addr
                );
            }
            ...
            if is_dangerous && counter.is_multiple_of(10) {
                let reason = if proc_name == "nmap.exe" {
                    "Port Scan Detected"
                } else {
                    "Suspicious Payload"
                };
